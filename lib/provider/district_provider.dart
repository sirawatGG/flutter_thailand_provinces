...
class DistrictProvider {
  ...
  static Future<List<DistrictDao>> searchInAmphure({int amphureId = 1, String keyword = ""}) async {
    List<Map<String, dynamic>> mapResult = await ThailandProvincesDatabase.db.query(TABLE_DISTRICT,
        where: "(amphure_id = ?) AND ( name_th LIKE ? OR name_en LIKE ? )", whereArgs: ["$amphureId", "%$keyword%", "%$keyword%"]);
