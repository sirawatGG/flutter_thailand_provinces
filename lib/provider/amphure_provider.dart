...
class AmphureProvider {
  ...
  static Future<List<AmphureDao>> searchInProvince({int provinceId = 1, String keyword = ""}) async {
    List<Map<String, dynamic>> mapResult = await ThailandProvincesDatabase.db.query(TABLE_AMPHURES,
        where: "(province_id = ?) AND ( name_th LIKE ? OR name_en LIKE ? )", whereArgs: ["$provinceId", "%$keyword%", "%$keyword%"]);
  ...
  static List<AmphureDao> mapAmphuresList(List<Map<String, dynamic>> mapResult) {
    ...
    for (Map<String, dynamic> mapRow in mapResult) {
